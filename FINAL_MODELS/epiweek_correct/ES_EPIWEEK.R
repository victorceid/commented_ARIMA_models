...
ES_EPIWEEK<-function(current_state_tb, auto=FALSE, n_weeks_ahead=1, ES27=TRUE, ES64=FALSE, window=104,week_lag=0){
  ...
  results[[1]]%<-% ROLLING_ARIMAX_EPIWEEK(current_state_tb, n_weeks_ahead=n_weeks_ahead, window = window, week_lag=week_lag, order_params=my_order_params, auto=auto) %packages% "forecast" 
...
ROLLING_ARIMAX_EPIWEEK <- function(current_state_tb, n_weeks_ahead=1, window = 104, order_params=NULL, auto=FALSE, week_lag=week_lag) {
  ...
  for(iter in  (1+week_lag):(NROW(current_state_tb$cases)-(window))){ 
    ...
    # We recalculate the mean cases and use the last date
    exog_var_dataset= data.frame(current_state_tb[current_state_rolling_window,])
    exog_var_<-(rolling_epiweek_data$cases[1:52] + rolling_epiweek_data$cases[53:104])/2
    exog_var<-c(exog_var_[52], exog_var_[52], exog_var_[52], exog_var_[52])
