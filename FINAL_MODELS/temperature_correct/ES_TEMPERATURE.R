...
ES_TEMPERATURE<-function(current_state_tb, week_lag=0, temperature_data=NULL, auto=FALSE, n_weeks_ahead=1, ES27=TRUE, ES64=FALSE, window=104){
  ...
  results[[1]]%<-% ROLLING_ARIMAX_TEMP(current_state_tb, temperature_data=temperature_data, week_lag=week_lag, n_weeks_ahead=n_weeks_ahead, window = window, order_params=my_order_params, auto=auto) %packages% "forecast" 
...
#' @return A list containing the forecast results and performance metrics.
#'
ROLLING_ARIMAX_TEMP <- function(current_state_tb, week_lag=week_lag, temperature_data=NULL, n_weeks_ahead=1, window = 104, order_params=NULL, auto=FALSE) {
  ...
  ####################################################################
  # Iterations, current_state_rolling_window and exog_rolling_window #
  # The model iterates over the ILI dataset based on the 'lag_week' that we want to use on the ARIMAX exogenous dataset.
  # It also take into account our defined window (104 weeks), and the number of rows (N_of_weeks) of the ILI dataset.
  # For week_lag = 1, we fit the current state data in a (104) weeks window beginning on the day two, e.g. [2:105]
  # This is done because we need the exogenous_rolling_window to start on day one, e.g. [1:104]
  # If lag_week > 1, we keep the exogenous_rolling_window on 1:104, and move the current_state_rolling_window a week ahead. 
  # For instance, lag_week=2, current_state_rolling_window= 3:106, exogenous_rolling_window on 1:104.
  # If lag_week = 0, the exogenous dataset is fitted for the same dates as the current state data.
  # We will have less iterations for higher lag_week values because we need more initial weeks for the exog_rolling_window.
  # When calculating the overall performance of the model is important to compare the results of the same dates.
  #####################################################################################################################
...
##############
      ...
      for(my_model in fitted_models){
        ...
        if(length(my_model)>0 && !is.na(my_model[1]) && !(is.na(my_model$aic))){ 
          ...
          #####################################
          # get the prediction interval levels
          pi<-forecast(my_model, h = n_weeks_ahead, xreg=exog_var[1:n_weeks_ahead], level =  c(0.01, 0.025, seq(0.05, 0.95, by = 0.05), 0.975, 0.99))
        }
      }
